...
int waitTime, downTime, throughTime, upTime;
...
void loop() {
  ...
  // We will try to navigate the obstacle course by first going straight down,
  // then running the vertical thruster at a low speed to maintain depth while
  // running the side thrusters at full speed to go through the hoop, and then
  // going straight up.

  if (currentTime < waitTime) { // wait for 15 seconds to let us seal the box
