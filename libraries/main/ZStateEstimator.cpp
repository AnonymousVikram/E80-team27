...
void ZStateEstimator::init(void) { state.z = 0; }
...
void ZStateEstimator::updateState(int pressure_signal) {
  ...
  state.z = depthCal_slope * pressure_voltage +
            depthCal_intercept; // convert from Volts to depth [m]

  // uncomment the following print statement to calibrate your pressure sensor
  // with the Teensy using the Serial Monitor String calibrationMessage =
  // "Pressure Sensor Voltage: " + String(pressure_voltage);
