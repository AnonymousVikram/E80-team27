...
void DepthControl::init(const int totalWayPoints_in, double *wayPoints_in,
                        int diveDelay_in) {
  totalWayPoints = totalWayPoints_in;
  // create wayPoints array on the Heap so that it isn't erased once the main
  // Arduino loop starts
...
void DepthControl::dive(z_state_t *state, int currentTime_in) {
  ...
  // Set the value of depth_des, depth, vertical control effort (uV)
  // appropriately for P control You can access the desired depth from the
  // wayPoints array at the index held in currentWayPoint
  depth_des = wayPoints[currentWayPoint];
  // You can access the measured depth calculated in ZStateEstimator.cpp using
  // state->z
  depth = state->z;
  depth_error = depth_des - depth;

  // Set the vertical control effort (uV) to a value that will drive the depth
  // error to zero usig K
  uV = Kp * depth_error;
  // limit uV to +- 200
  if (uV > 200)
    uV = 200;
...
void DepthControl::updatePoint(float z) {
  ...
  if ((depth_error < DEPTH_MARGIN && currentWayPoint < totalWayPoints) ||
      delayed) {
