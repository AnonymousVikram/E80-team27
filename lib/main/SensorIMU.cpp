...
void SensorIMU::read(void) {
  ...
  state.magX = mx * mag_ironcomp[0][0] + my * mag_ironcomp[0][1] +
               mz * mag_ironcomp[0][2];
  state.magY = mx * mag_ironcomp[1][0] + my * mag_ironcomp[1][1] +
               mz * mag_ironcomp[1][2];
  state.magZ = mx * mag_ironcomp[2][0] + my * mag_ironcomp[2][1] +
               mz * mag_ironcomp[2][2];

  // populate the roll, pitch, yaw with simple orientation calcs
  getOrientation(state.accelX, state.accelY, state.accelZ, state.magX,
                 state.magY, state.magY);
}
...
std::string SensorIMU::logData(void) {
  std::string logString = "";
  logString += std::to_string(state.accelX) + ",";
  logString += std::to_string(state.accelY) + ",";
  logString += std::to_string(state.accelZ) + ",";
  logString += std::to_string(state.magX) + ",";
  logString += std::to_string(state.magY) + ",";
  logString += std::to_string(state.magZ) + ",";
  logString += std::to_string(state.roll) + ",";
  logString += std::to_string(state.pitch) + ",";
  logString += std::to_string(state.heading) + ",";
  return logString;
}

void SensorIMU::getOrientation(float ax, float ay, float az, float mx, float my,
                               float mz) {
  ...
  if (ay * sin(state.roll) + az * cos(state.roll) == 0)
    ...
    state.pitch =
        (float)atan(-ax / (ay * sin(state.roll) + az * cos(state.roll)));
