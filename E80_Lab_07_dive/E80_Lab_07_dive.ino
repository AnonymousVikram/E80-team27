...
void setup() {
  ...
  int diveDelay = 10000; // how long robot will stay at depth waypoint before
                         // continuing (ms)

  const int num_depth_waypoints = 2;
  double depth_waypoints[] = {0.3, 0.7}; // listed as z0,z1,... etc.
  ...
  button_sampler.lastExecutionTime =
      loopStartTime - LOOP_PERIOD + BUTTON_LOOP_OFFSET;
  xy_state_estimator.lastExecutionTime =
      loopStartTime - LOOP_PERIOD + XY_STATE_ESTIMATOR_LOOP_OFFSET;
  z_state_estimator.lastExecutionTime =
      loopStartTime - LOOP_PERIOD + Z_STATE_ESTIMATOR_LOOP_OFFSET;
  depth_control.lastExecutionTime =
      loopStartTime - LOOP_PERIOD + DEPTH_CONTROL_LOOP_OFFSET;
  ...
  if (currentTime - depth_control.lastExecutionTime > LOOP_PERIOD) {
    ...
    if (depth_control.surfaceState) { // SURFACE STATE //
      if (!depth_control.atSurface) {
        ...
        delete[] depth_control
            .wayPoints; // destroy depth waypoint array from the Heap
      }
      motor_driver.drive(0, 0, depth_control.uV);
    }
  }
